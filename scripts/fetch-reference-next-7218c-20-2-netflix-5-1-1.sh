...
######### Edit directory below where to find 20.2 tarballs
export TGZS_DIR=/shared/rdk/20.2/
#########
 
######### Build setup and repo sync
rm -rf 7218c_reference_20_2_netflix$TARGET_DIR_SUFFIX
mkdir 7218c_reference_20_2_netflix$TARGET_DIR_SUFFIX
cd 7218c_reference_20_2_netflix$TARGET_DIR_SUFFIX
...
(cd meta-cmf-video-reference-next && git fetch "https://code.rdkcentral.com/r/components/generic/rdk-oe/meta-cmf-video-reference-next" refs/changes/93/49193/2 && git cherry-pick FETCH_HEAD)
...
# create nrd package from LGI repo
git clone git@github.com:LibertyGlobal/onemw-netflix.git && \
(cd onemw-netflix && git checkout netflix_5.1.1_poc &&  git checkout c3596cfb748bfcbb9afa18eefc220af48861655d && tar cz --exclude=".git" -f ../downloads/nrd-5.1.1-1340856.tar.gz . ) && \
rm -rf onemw-netflix
...
echo "RUN FOLLOWING TO PREPARE FOR BUILD: cd 7218c_reference_20_2_netflix$TARGET_DIR_SUFFIX; source _build.sh"
